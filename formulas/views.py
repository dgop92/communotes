...
from formulas.custom_filters import (PhotoClassificationFilter, PhotoFilter,
                                     ReviewFilter)
from formulas.custom_permissions import IsCurrentUserOwnerOrReadOnly
from formulas.docs import (photo_classification_docs, photo_context_docs,
                           photo_docs, profile_docs, review_docs, subject_docs,
                           tag_docs)
...
from formulas.serializers import (PhotoClassificationSerializer,
                                  PhotoContextSerializer, PhotoSerializer,
                                  ProfileSerializer, ReviewSerializer,
                                  SubjectSerializer, TagSerializer)


@extend_schema_view(
    **photo_docs.custom_schema.get_list_view_schema()
)
...
@extend_schema_view(
    **photo_docs.custom_schema.get_detail_view_schema()
)
...
@extend_schema_view(
    **photo_classification_docs.custom_schema.get_list_view_schema()
)
...
@extend_schema_view(
    **photo_classification_docs.custom_schema.get_detail_view_schema()
)
...
class ReviewList(generics.ListCreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        try:
            ...
            return Response(
                {"non_field_errors": [
                    _("an user can only make one review per photo")
                ]}, 
