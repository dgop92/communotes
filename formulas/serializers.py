...
from formulas.models import (Photo, PhotoClassification, PhotoContext, Profile,
                             Review, Subject, Tag)
from formulas.serializer_extra_fields import (HybridImageField,
...
def validate_subject_slug(value):
    if not re.match(slug_re, value):
        raise serializers.ValidationError(_("Enter a valid “slug” consisting of letters, numbers, underscores or hyphens."))
...
class PhotoSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        photo_classification_new_data = validated_data.get(
        ...
        if 'photo_context' in validated_data:
            ...
            if instance.photo_context:
                instance.photo_context.professor = photo_context_data.get(
                    'professor', 
                    instance.photo_context.professor
                )
                instance.photo_context.formula_type = photo_context_data.get(
                    'formula_type', 
                    instance.photo_context.formula_type
                )
                instance.photo_context.save()
            else:
                instance.photo_context = PhotoContext \
                    .objects.create(**photo_context_data)
        ...
        if 'photo' in data:
            ...
            if user == data['photo'].user:
                raise serializers.ValidationError(
                    _("Photo owner cannot make a review")
                )
