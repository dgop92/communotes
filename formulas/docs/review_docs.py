...
owner_review_error_examples = OpenApiExample(
    ...
    value={"non_field_errors": ["Photo owner cannot make a review"]},
    response_only=True,
    status_codes=["400"],
)
...
duplicate_review_error_example = OpenApiExample(
    ...
    value={"non_field_errors": ["an user can only make one review per photo"]},
    response_only=True,
    status_codes=["400"],
)

p1 = OpenApiParameter(name="photo", description="Photo pk", type=OpenApiTypes.INT)
...
class CustomReviewSchema(CustomSchemaHelper):
    ...
    def get_custom_post_examples(self):
        return [
            owner_review_error_examples,
            duplicate_review_error_example,
        ]
