...
class PhotoClassification(models.Model):
    subject = models.ForeignKey(
        Subject, on_delete=models.CASCADE, related_name="photo_classifications"
    )
...
class PhotoContext(models.Model):
    ...
    formula_type = models.CharField(
        max_length=2, choices=FormulaType.choices, blank=True
    )
...
class Photo(TimestampedModel):
    ...
    photo_context = models.OneToOneField(
        PhotoContext, on_delete=models.CASCADE, related_name="photo", null=True
    )
...
class Review(models.Model):
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["photo", "user"], name="one review per photo"
            )
        ]
