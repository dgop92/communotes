...
from formulas.models import (Photo, PhotoClassification, PhotoContext, Subject,
                             Tag, user_photos_directory_path)
...
class PhotoTest(CrudTestBase, TestListViewMixin, TestDetailViewMixin):
    ...
    def get_object_list(self):
        photos = PhotoFactory.create_batch(
            3, file=None, user=self.get_owner_user()
        )
        return photos
    ...
    def test_upload_photo(self):
        ...
        self.assertTrue(default_storage.exists(
            user_photos_directory_path(photo, filename)
        ))
    ...
    def test_subject_filter(self):
        ...
        compare_subjects = lambda s: s == subject_name
    ...
    def test_photo_classification_filter(self):
        ...
        compare_cl = lambda pc: pc == photo_classification.pk
    ...
    def test_user_filter(self):
        photo1 = PhotoFactory.create(file=None, user=self.get_owner_user())
        PhotoFactory.create(
            file=None, user=self.normal_users[1]['user']
        )
    ...
    def test_tags_filter(self):
        ...
        PhotoFactory.create(
            file=None, user=self.get_owner_user(), tags=(t1, )
        )
        PhotoFactory.create(
            file=None, user=self.get_owner_user(), tags=(t2, )
        )
        p3 = PhotoFactory.create(
            file=None, user=self.get_owner_user(), tags=(t1, t2)
        )
        query1 = [('tag', t1.name)]
