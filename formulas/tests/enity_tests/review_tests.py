...
from core.tests.mixins import TestListViewMixin, TestDetailViewMixin
...
class ReviewTest(CrudTestBase, TestListViewMixin, TestDetailViewMixin):
    ...
    def get_object_list(self):
        ...
        for user_dict in self.normal_users[1:]:
            user = user_dict['user']
            obj_list.append(
                ReviewFactory.create(photo=self.photos[0], user=user)
            )
        return obj_list
    
    def get_detail_object(self):
        review = self.get_model_factory().create(
            photo=self.photos[0], user=self.normal_users[1]['user'],
        )
        return review
