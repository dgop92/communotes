...
class TestCreateMixin:
    def test_create(self):
        ...
        self.post(self.get_list_url(), **kwargs)
        self.assert_instance_exists(self.get_model(), pk=self.json_response["pk"])
...
class TestListMixin:
    def test_list(self):
        ...
        self.get(self.get_list_url(), **kwargs)
...
class TestRetrieveMixin(ObjectBaseMixin):
    def test_retrieve(self):
        ...
        self.get(self.get_detail_url(pk=pk), **kwargs)
...
class TestUpdateMixin(ObjectBaseMixin):
    def test_update(self):
        ...
        if put_data:
            kwargs = {"status_code": status.HTTP_200_OK, "data": put_data}
            kwargs.update(self.get_request_kwargs())
            kwargs.update(self.get_extra_update_kwargs())
            self.put(self.get_detail_url(pk=pk), **kwargs)
            self.assert_dict_contains_subset(self.json_response, put_data)

        patch_data = self.get_data_for_patch()
        if patch_data:
            kwargs = {"status_code": status.HTTP_200_OK, "data": patch_data}
            kwargs.update(self.get_request_kwargs())
            kwargs.update(self.get_extra_update_kwargs())
            self.patch(self.get_detail_url(pk=pk), **kwargs)
...
class TestDeleteMixin(ObjectBaseMixin):
    def test_delete(self):
        ...
        self.delete(self.get_detail_url(pk=pk), **kwargs)
        self.assert_instance_does_not_exist(self.get_model(), pk=pk)
